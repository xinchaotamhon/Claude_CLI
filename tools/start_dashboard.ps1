...
function Test-DashboardIdentity {
    try {
        ...
        if ([string]::IsNullOrWhiteSpace([string]$ReadyState.instanceId)) { return $false }
        ...
        return $Response.ok -eq $true -and $Response.service -eq 'claude-cli-dashboard' -and [string]$Response.instanceId -eq [string]$ReadyState.instanceId
    }
    catch { return $false }
}
